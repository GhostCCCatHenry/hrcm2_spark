...
/*            FileSystem fs = FileSystem.get(conf);
    ...
    private static compression_type createSeqRDD(Iterator<String> s){
...
//         ("seqCodeMatching complete. MatchResult size: "+mr.size());
        return mr;
    }
    ...
    //计算matchResult的hash值
    private static void matchResultHashConstruct(List<MatchEntry> matchResult,List<int[]> seqBucketVec,List<List<Integer>> seqLocVec) {
...
/*        list.add(String.valueOf(matchEntry.getLength()));
    ...
    private static void codeSecondMatch( List<MatchEntry> _mr, int seqNum, List<int[]> seqBucket_vec,
                                         List<List<Integer>> seqLoc_vec , List<List<MatchEntry>> matchResult_vec,
                                         List<String> list) {
        int hashValue;
        int pre_seq_id=1;
        ...
        for (i = 0; i < _mr.size()-1; i++) {
            ...
            max_length = 0;
            //寻找相同序列
            for (int m = 0; m < min( seqNum-1, 110 ); m++) {
        ...
        if (!misMatchEntry.isEmpty()) {
            for (int j = 0; j < misMatchEntry.size(); j++)
                saveMatchEntry(list, misMatchEntry.get(j));
            misMatchEntry.clear();
        }
        System.out.println("code second match complete. The second match length: "+secondMatchTotalLength);
    }

    public static void compress(String ref_file,String tar_file){
...
//        String masterurl = "spark://10.166.38.112:7077";
        sparkConf.set("spark.kryo.registrator", mykryo.class.getName()).setAppName("App").setMaster("local[*]");
        sparkConf.set("spark.kryoserializer.buffer.mb","10").set("spark.kryoserializer.buffer.max","512").
                set("spark.storage.memoryFraction", "0.5");
        /*sparkConf.set("spark.default.parallelism", "20");*/
        sparkConf.set("spark.default.parallelism", "30").set("spark.shuffle.file.buffer","64").set("spark.reducer.maxSizeInFlight", "96").set("spark.shuffle.io.maxRetries", "6");
        ...
        Path path1 = new Path("J:\\gene\\output\\spark\\out1");
//        Path path1 = new Path("/output/spark/out1");
        Path path2 = new Path("J:\\gene\\output\\spark\\out2");
//        Path path2 = new Path("/output/spark/out2");
//        Path ref_path = new Path(ref_file);
        /*
            使用FileSystemAPI
            读取参考序列以及清理输出位置
        * */
...
//            FSDataInputStream in = fs.open(file);
        ...
        JavaRDD<compression_type> read_tar = tar_rdd.mapPartitions(s-> {
                    /*
                    ...
                    //封装进去 完成输出
                    compression_type tar = createSeqRDD(s);
                    list.add(tar);
        ...
        JavaRDD<Tuple2<Integer,List<MatchEntry>>> first_Res = read_tar.mapPartitionsWithIndex((v1,v2)->{
            List<Tuple2<Integer,List<MatchEntry>>> list = new ArrayList<>();
            Tuple2<Integer,List<MatchEntry>> tu2 =null;
            while(v2.hasNext())
                tu2 = new Tuple2<>(v1,codeFirstMatch(v2.next(),referenceTypeBroadcast.value()));
            list.add(tu2);
            return list.iterator();
...
//        read_tar.unpersist();//不需要缓存了
        JavaRDD<Tuple3<List<List<MatchEntry>>,List<int[]>,List<List<Integer>>>> sec = first_Res.filter(s-> (s._1<=110)).coalesce(1,true).mapPartitions(s->{
            List<Tuple3<List<List<MatchEntry>>,List<int[]>,List<List<Integer>>>> li = new ArrayList<>();
            List<List<MatchEntry>> list = new ArrayList<>();
            List<MatchEntry> l ;
            List<int[]> seqBucketVec = new ArrayList<>();
            List<List<Integer>> seqLocVec = new ArrayList<>(); //存储所有序列的冲突元素
//            Map<Integer,List<MatchEntry>> map ;
            while (s.hasNext()){
                l = s.next()._2;
//                map = s.next();
                list.add(l);
                matchResultHashConstruct(l,seqBucketVec,seqLocVec);
            }
            li.add(new Tuple3<>(list,seqBucketVec,seqLocVec));
            return li.iterator();
        });
...
/*        List<List<MatchEntry>> sec1 = new ArrayList<>();
        ...
        final Broadcast<Tuple3<List<List<MatchEntry>>,List<int[]>,List<List<Integer>>>> sec_ref = jsc.broadcast(sec.first());
/*
...
//        final Broadcast<Tuple2<List<int[]>,List<List<Integer>>>> sec_ref2 = jsc.broadcast(sec2.first());
        ...
        first_Res.mapPartitions(s->{
            List<String> list = new ArrayList<>();
//            Tuple3<Integer,List<MatchEntry>,List<String>> tu3;
            Tuple2<Integer,List<MatchEntry>> tar;
            //根据分区号来进行筛选
            while (s.hasNext()) {
                tar = s.next();
